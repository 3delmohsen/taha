...
const MainApp: React.FC = () => {
  ...
  const renderView = () => {
    switch (currentView) {
      ...
      case AppView.QURAN:
        return <QuranView onToggleFullScreen={setIsFullScreen} />;
      case AppView.FATWA:
        return <AIFatwaView />;
      case AppView.HAJJ:
      ...
      case AppView.DUA:
        return <DuaView goBack={() => setView(AppView.HOME)} />;
      case AppView.QIBLA:
        return <QiblaView goBack={() => setView(AppView.HOME)} />;
  ...
  return (
    <div className="min-h-screen bg-slate-100 text-slate-900 selection:bg-emerald-100 selection:text-emerald-900 flex justify-center" dir={dir}>
      <div className="w-full max-w-md bg-[#fdfbf7] shadow-2xl shadow-slate-300 relative flex flex-col h-[100dvh]">
         {/* Top Status Bar Decoration */}
         <div className="h-1.5 bg-gradient-to-r from-emerald-500 via-amber-400 to-emerald-600 w-full z-50 shrink-0"></div>
         ...
         {/* Bottom Navigation */}
         {!isFullScreen && <Navigation currentView={currentView} setView={setView} />}
      </div>
    </div>
  );
};
